...
| Name | Status | Value | Justification |
|---|---|---|---|
| `_N_DAVIES_MANDOUVALOS` | ✅ | `3` | Intrinsic dim with the Davies–Mandouvalos (1988) closed-form heat kernel `k^3_t(d) = (4πt)^{-3/2} exp(-t - d²/4t) d/sinh d`. **Derived**: 3 is fixed by the mathematics — the lowest dim where the Grigor'yan–Noguchi recursion gives an elementary closed form starting from H^1 (the Gaussian). Used in `src/holonomy_lib/hyperbolic/heat_kernel.py` as the recursion seed for higher odd dims and as the dispatch target for n=3. Cataloged 2026-05-27. |
